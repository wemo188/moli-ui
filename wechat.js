...
(function() {
  ...
  var Wechat = {
    ...
    render: function() {
      ...
      panel.innerHTML =
'<div class="' + wrapClass + '" id="wxWrap" style="display:flex;flex-direction:column;align-items:center;height:100%;"><div class="wx-phone"><div class="wx-inner">' +
          '<div class="wx-header">' +
...
'<button class="wx-me-mode-btn" id="wxModeToggle" type="button" style="display:none;position:relative;z-index:1;">' +
    ...
    renderTab: function() {
      ...
      if (Wechat.currentTab === 'me') {
        ...
        if (modeBtn) modeBtn.style.display = '';} else {
      ...
      body.innerHTML = chars.map(function(c) {
        ...
        return '<div class="wx-chat-item" data-char-id="' + c.id + '">' +
          ...
          '<div class="wx-chat-content">' +
            ...
            '<div class="wx-chat-msg">点击开始聊天</div>' +'</div>' +
        '</div>';
      }).join('');
    ...
    /* ★ 修改：通讯录过滤掉 contactMode 为 wait 和 manual 的角色 */
    renderCharTab: function(body) {
      var chars = (App.character ? App.character.list : []).filter(function(c) {
        return !c.contactMode || c.contactMode === 'direct';
      });
      if (!chars.length) {
        body.innerHTML = '<div class="wx-empty"><svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg><div class="wx-empty-text">暂无联系人</div></div>';
        return;
      }
      body.innerHTML = chars.map(function(c) {
        /* ★ 修改：img src 用 escAttr */
