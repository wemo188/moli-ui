...
(function() {
  ...
  var Wechat = {
    ...
    bindEvents: function() {
      ...
      var wxPhone = wxInner ? wxInner.closest('.wx-phone') : null;
      if (wxInner) {
        var _wxSwipe = { active: false, sx: 0, sy: 0, locked: false, dir: '' };
        var swipeTarget = wxPhone || wxInner;
        ...
        wxInner.addEventListener('touchmove', function(e) {
          ...
          if (_wxSwipe.dir === 'h' && dx > 0) {
            e.preventDefault();
            var w = swipeTarget.offsetWidth || window.innerWidth;
            swipeTarget.style.transform = 'translateX(' + Math.min(dx, w) + 'px)';
            swipeTarget.style.opacity = String(1 - dx / w * 0.5);
          }
        }, { passive: false });
        ...
        wxInner.addEventListener('touchend', function(e) {
          ...
          if (_wxSwipe.dir !== 'h') { swipeTarget.style.transform = ''; swipeTarget.style.opacity = ''; return; }
          var t = e.changedTouches[0];
          var dx = t.clientX - _wxSwipe.sx;
          var w = swipeTarget.offsetWidth || window.innerWidth;
          if (dx > w * 0.3) {
            swipeTarget.style.transition = 'transform .25s ease, opacity .25s ease';
            ...
            swipeTarget.style.transition = 'transform .2s ease, opacity .2s ease';
            swipeTarget.style.transform = '';
            swipeTarget.style.opacity = '';
            setTimeout(function() { swipeTarget.style.transition = ''; }, 220);
          }
        }, { passive: true });
      }
    },
