  ...
  var Wechat = {
      ...
      body.innerHTML = visibleChars.map(function(c) {
        ...
        return '<div class="wx-chat-item" data-char-id="' + c.id + '">' +
          ...
          '<div class="wx-chat-content">' +
            '<div class="wx-chat-top"><span class="wx-chat-name">' + App.esc(c.name || '未命名') + '</span><span class="wx-chat-time">' + lastTime + '</span></div>' +
    ...
    renderMeTab: function(body) {
      ...
      body.innerHTML =
        ...
        '</div>' +
        '<div>' +
          '<div class="wx-me-link" id="wxMeMode">' +
            '<span class="wx-me-link-text">显示模式</span>' +
            '<span style="font-size:13px;color:#8aa0b8;">' + (isFS ? '全屏' : '手机框') + ' ›</span>' +
          '</div>' +
      ...
      body.querySelector('#wxMeFavs').addEventListener('click', function() {
        ...
        var html = favs.map(function(f, i) {
          return '<div style="padding:12px;border-bottom:1px solid rgba(0,0,0,.04);font-size:13px;color:#333;line-height:1.5;">' +
            '<div style="font-size:11px;color:#999;margin-bottom:4px;">' + App.esc(f.charName || '') + '</div>' +
            '<div>' + App.esc((f.content || '').slice(0, 100)) + '</div>' +
          '</div>';
        }).join('');
        var fBody = App.$('#wxBody');
        if (fBody) fBody.innerHTML = '<div style="padding:10px 0;">' + html + '</div>';
      });
    },
