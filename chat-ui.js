...
'<div class="ct-input-wrap">'+
  ...
  '<button class="ct-send" id="ctSend" type="button">'+SEND_SVG+'</button>'+
...
Chat.messages.forEach(function(msg,idx){
  ...
  var bubbles=(msg.content||'').split(utils.SPLIT);
  ...
  bubbles.forEach(function(text,bIdx){
    ...
    if(stickerMatch){
      ...
      var stickerImgUrl=App.LS.get('stickerCache_'+desc.replace(/\s+/g,'_').slice(0,30));
      if(stickerImgUrl){
        bubbleContent+='<img class="ct-sticker" src="'+App.escAttr(stickerImgUrl)+'" alt="sticker">';
      } else {
        var stickerId='stk_'+idx+'_'+bIdx;
...
['piPhoto','piSticker','piVoiceMsg','piVoiceCall','piVideoCall','piRedPacket','piTransfer','piLocation'].forEach(function(id){
  App.safeOn('#'+id,'click',function(e){
    e.stopPropagation();
    var labels={piPhoto:'图片',piSticker:'表情包',piVoiceMsg:'语音消息',piVoiceCall:'语音通话',piVideoCall:'视频通话',piRedPacket:'红包',piTransfer:'转账',piLocation:'位置'};
...
else{btn.classList.remove('stop');btn.innerHTML=SEND_SVG;}
},
...
if(isUser){
  ...
  var cfg=Chat._utils.getCfg(Chat.charId);
  if(cfg.fallbackTTS||cfg.minimax)items+='<div class="ct-ctx-item" data-act="speak">'+(Chat._currentTTS?'停止播放':'播放语音')+'</div>';
}
items+='<div class="ct-ctx-item" data-act="delafter">往后全删</div>';
items+='<div class="ct-ctx-item" data-act="del">删除</div>';
menu.innerHTML=items;

var left=Math.min(x,window.innerWidth-140),top=Math.min(y-10,window.innerHeight-250);if(top<60)top=60;
...
menu.querySelectorAll('.ct-ctx-item').forEach(function(item){
  item.addEventListener('click',function(e){
    ...
    else if(act==='speak')Chat.speakMsg(idx);
  });
});
},
