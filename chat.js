...
还有一个方案：**两种都支持，优先识别 `|||`，如果没有 `|||` 就按双换行拆**。这样 AI 不管用哪种方式都能正确拆分。

我就用这个方案。

### 关于第2点

你说的"检测用户离开输入框"，在技术上就是监听输入框的 `blur` 事件（用户点了别的地方）和"5秒无输入"的 idle 检测。这个方案比固定等3秒更聪明。

### 关于第3点

用角色名+称呼嵌入规则更好，AI 会更入戏。

确认完毕，我开始写。文件太长我分段发，先发上半部分（辅助函数到 `buildApiMessages`），再发下半部分（Chat对象到底）。
