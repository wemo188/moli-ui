...
var proPrompt='';
if(lastIsUser){
  proPrompt='用户'+minsSince+'分钟前发了消息，你还没回复。你必须回复。请根据用户最后说的内容来回复，不要忽略它。如果用户问了问题就回答问题，如果用户在聊某个话题就接着聊。直接发消息，不允许回复[SKIP]。';
} else if(lastIsMe){
  if(minsSince<10){
    proPrompt='你'+minsSince+'分钟前刚发过消息，用户还没回。你刚说完话不久，自然地补一句就好。可以是：补充一个想法、发个表情包、突然想到一件事分享一下、或者追问一句。直接发消息内容。不允许回复[SKIP]。';
  } else if(minsSince<30){
    proPrompt='距离你上次发消息已经'+minsSince+'分钟了，用户没回。你可以：追问一下对方在干嘛、分享你现在在做的事、发个表情包试探一下、或者换个新话题聊。直接发消息内容。不允许回复[SKIP]。';
  } else {
    proPrompt='已经过了'+minsSince+'分钟了，用户一直没回。根据你的角色性格，自然地再说一句。可以是关心对方去哪了、吐槽对方不回消息、分享一件刚发生的事、或者发个无聊的日常。如果角色性格确实非常高冷且绝不会主动找人，才可以回复[SKIP]。大多数情况下你应该发消息。';
  }
} else {
  proPrompt='这是对话开始。你必须主动打招呼或找一个话题开聊。根据角色性格决定打招呼的方式（热情/随意/高冷/撒娇都行），但必须发消息。不允许回复[SKIP]。';
}

proPrompt+='\n\n【重要规则】\n1. 仔细阅读上面的完整对话历史再决定说什么。\n2. 不要无视用户还没被回答的问题。\n3. 不要在用户没回复时连续换话题。\n4. 绝大多数情况下你必须发消息。[SKIP]只在极端情况下使用（角色性格极度高冷+已经过了30分钟以上）。\n5. 回复必须符合角色性格。\n6. 不要发"在吗""你好"这种废话，要有实际内容。';
