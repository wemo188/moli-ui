(function() {
  ...
  var Api = {
    ...
    currentTab: 'config',
    _editingIdx: -1,
    _drag: { active: false, locked: false, sx: 0, sy: 0, ox: 0, oy: 0 },
    _closeListener: null,
    ...
    tkBtn: function(id, label) {
      return '<div class="api-fl-tk" id="' + id + '"><div class="api-fl-tk-body"><div class="api-fl-tk-inner"></div><div class="api-fl-tk-text">' + label + '</div></div></div>';
    },
    ...
    renderFloat: function() {
      ...
      if (Api.currentTab === 'config') {
        ...
        bottomHtml = '<div class="api-fl-bottom">' + Api.tkBtn('apiSaveBtn', '保存') + Api.tkBtn('apiCloseBtn', '退出') + '</div>';
        ...
        bottomHtml = '<div class="api-fl-bottom">' + Api.tkBtn('apiSaveParamsBtn', '保存') + Api.tkBtn('apiCloseBtn2', '退出') + '</div>';

      } else if (Api.currentTab === 'saved') {
        bodyHtml = Api.buildSavedHtml();
        bottomHtml = '<div class="api-fl-bottom">' + Api.tkBtn('apiCloseBtn3', '退出') + '</div>';
      }
    ...
    bindEvents: function() {
      ...
      el.addEventListener('touchend', function() { Api._drag.active = false; Api._drag.locked = false; _startedOnInput = false; });
