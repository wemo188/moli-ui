...
var Preset={
  ...
  load:function(){
    ...
    if(!Preset.presetList||!Preset.presetList.length){
      Preset.presetList=[makeDefaultConfig('默认预设')];
      Preset.activeIdx=0;
    }
  ...
  _renderUserItem:function(id,up,isActive){
    ...
    if(isActive){
      ...
      return '<div class="ps-item is-user" data-id="'+id+'"><div class="ps-drag">'+DRAG_SVG+'</div><div class="ps-info"><div class="ps-name">'+App.esc(up.name||'未命名')+'</div><div class="ps-desc">'+App.esc((up.content||'').slice(0,40))+'</div></div>'+depthTag+'<div class="ps-mini-btn edit" data-edit-id="'+id+'">'+EDIT_SVG+'</div><div class="ps-mini-btn del" data-del-id="'+id+'">'+DEL_SVG+'</div><div class="ps-sw '+(on?'on':'off')+'" data-usw-id="'+id+'"></div></div>';
    }else{
      return '<div class="ps-item is-user is-inactive" data-id="'+id+'"><div class="ps-drag" style="opacity:.25;">'+DRAG_SVG+'</div><div class="ps-info" style="opacity:.5;"><div class="ps-name">'+App.esc(up.name||'未命名')+'</div><div class="ps-desc">'+App.esc((up.content||'').slice(0,40))+'</div></div>'+depthTag+'<div class="ps-mini-btn edit" data-edit-id="'+id+'">'+EDIT_SVG+'</div><div class="ps-mini-btn del" data-del-id="'+id+'">'+DEL_SVG+'</div><div class="ps-sw off" data-usw-id="'+id+'"></div></div>';
    }
  },
  ...
  renderHome:function(home){
    ...
    list.forEach(function(cfg,idx){
      ...
      cardsHtml+='<div class="ps-home-card'+(isActive?' active-preset':'')+'" data-pidx="'+idx+'">'+
        '<div class="ps-home-card-info">'+
          '<div class="ps-home-card-name">'+App.esc(cfg.name||'未命名')+'</div>'+
          '<div class="ps-home-card-desc">'+userCount+' 条自定义指令</div>'+
        '</div>'+
        '<span class="ps-home-card-badge '+(isActive?'in-use':'idle')+'">'+(isActive?'使用中':'闲置')+'</span>'+
        '<div class="ps-home-actions">'+
          '<div class="ps-mini-btn" data-edit-idx="'+idx+'" title="编辑">'+EDIT_SVG+'</div>'+
          '<div class="ps-mini-btn" data-dots-idx="'+idx+'">'+DOTS_SVG+'</div>'+
        '</div>'+
      '</div>';
    });
    ...
    home.innerHTML=
      '<div class="ps-header">'+
        ...
        '<span class="ps-header-title">预设方案</span>'+
    ...
    home.querySelector('#psHomeAdd').addEventListener('click',function(){
      var newCfg=makeDefaultConfig('预设 '+(Preset.presetList.length+1));
      Preset.presetList.push(newCfg);
      Preset.activeIdx=Preset.presetList.length-1;
    ...
    // 编辑按钮进详情
    home.querySelectorAll('[data-edit-idx]').forEach(function(btn){
      btn.addEventListener('click',function(e){
        e.stopPropagation();
        var idx=parseInt(btn.dataset.editIdx);
        Preset.activeIdx=idx;
        Preset.config=Preset.presetList[idx];
        Preset.save();
        Preset.openDetail();
      });
    });
    ...
    home.querySelectorAll('.ps-home-card').forEach(function(card){
      card.addEventListener('click',function(e){
        ...
        Preset.save();
        Preset.renderHome(home);
        App.showToast('已启用「'+Preset.config.name+'」');
      });
    });
    ...
    home.querySelectorAll('[data-dots-idx]').forEach(function(btn){
      btn.addEventListener('click',function(e){
        e.stopPropagation();
        var idx=parseInt(btn.dataset.dotsIdx);
        Preset._showDotsMenu(idx,btn,home);
      });
    });

    Preset._bindHomeDrag(home);
    Preset._bindSwipeToEl(home,function(){Preset.closeHome();});
  },
  ...
  _showDotsMenu:function(idx,anchorEl,home){
    ...
    menu.innerHTML=
      '<div class="ps-dots-mi" data-dact="rename">重命名</div>'+
      '<div class="ps-dots-mi" data-dact="copy">复制</div>'+
      '<div class="ps-dots-mi" data-dact="export">导出</div>'+
      (Preset.presetList.length>1?'<div class="ps-dots-mi danger" data-dact="delete">删除</div>':'');
    ...
    menu.querySelectorAll('.ps-dots-mi').forEach(function(mi){
      mi.addEventListener('click',function(e){
        ...
        if(act==='rename'){
          var name=prompt('预设名称：',cfg.name||'');
          if(name===null)return;name=name.trim();
          if(name){cfg.name=name;Preset.save();Preset.renderHome(home);}
        }
        if(act==='copy'){
          var copy=JSON.parse(JSON.stringify(cfg));
          copy.name=(copy.name||'预设')+' (副本)';
          Preset.presetList.push(copy);
          Preset.save();Preset.renderHome(home);App.showToast('已复制');
        }
        if(act==='export'){
          var blob=new Blob([JSON.stringify(cfg,null,2)],{type:'application/json'});
          var url=URL.createObjectURL(blob);var a=document.createElement('a');
          a.href=url;a.download='preset-'+cfg.name+'.json';
          document.body.appendChild(a);a.click();document.body.removeChild(a);
          URL.revokeObjectURL(url);App.showToast('已导出');
        }
        if(act==='delete'){
          if(Preset.presetList.length<=1){App.showToast('至少保留一个方案');return;}
          if(!confirm('删除「'+cfg.name+'」？'))return;
          Preset.presetList.splice(idx,1);
          if(Preset.activeIdx>=Preset.presetList.length)Preset.activeIdx=0;
          Preset.config=Preset.presetList[Preset.activeIdx];
          Preset.save();Preset.renderHome(home);App.showToast('已删除');
        }
      });
    });

    setTimeout(function(){
      function dismiss(ev){if(!menu.parentNode)return;if(!menu.contains(ev.target)){menu.remove();document.removeEventListener('touchstart',dismiss);document.removeEventListener('click',dismiss);}}
  ...
  _bindHomeDrag:function(home){
    ...
    var dragEl=null,startY=0,startX=0,offsetY=0,dragIdx=-1,longPressed=false,timer=null,moved=false;
    var cardEls=[],cardRects=[];

    list.addEventListener('touchstart',function(e){
      var card=e.target.closest('.ps-home-card');if(!card)return;
      if(e.target.closest('.ps-mini-btn'))return;
      var t=e.touches[0];startX=t.clientX;startY=t.clientY;moved=false;longPressed=false;
      ...
      timer=setTimeout(function(){
        ...
        list.querySelectorAll('.ps-home-card').forEach(function(c){
          if(c===dragEl)return;
          cardEls.push(c);
          cardRects.push({idx:parseInt(c.dataset.pidx),mid:c.getBoundingClientRect().top+c.offsetHeight/2});
        });
    ...
    list.addEventListener('touchmove',function(e){
      ...
      if(!longPressed){
        if(Math.abs(t.clientY-startY)>8||Math.abs(t.clientX-startX)>8){clearTimeout(timer);timer=null;dragEl=null;}
        return;
      }
    ...
    list.addEventListener('touchend',function(e){
      ...
      for(var i=0;i<cardRects.length;i++){
        if(touchY<cardRects[i].mid){newIdx=cardRects[i].idx;break;}
      }
  ...
  closeDetail:function(){
    ...
    var home=App.$('#presetHomePage');if(home)Preset.renderHome(home);
  },
  ...
  renderList:function(page){
    ...
    orderedItems.forEach(function(item){
      if(item.type==='sys'){
        ...
        activeHtml+='<div class="ps-item is-sys" data-id="'+item.id+'">'+
          ...
          (hasSwitch?'<div class="ps-sw '+(on?'on':'off')+'" data-sw-id="'+item.id+'"></div>':'')+
        '</div>';
      }else{
    ...
    if(inactiveItems.length){
      ...
      inactiveHtml='<div style="padding:16px 18px;font-size:11px;color:#ccc;">暂无未激活指令</div>';
    }
    ...
    page.innerHTML=
      '<div class="ps-header">'+
        ...
        '<span class="ps-header-edit" id="psRenameBtn">重命名</span>'+
      '</div>'+
  ...
  bindListEvents:function(page){
    ...
    page.querySelectorAll('[data-edit-id]').forEach(function(btn){
      btn.addEventListener('click',function(e){
        e.stopPropagation();
        Preset.openEdit(btn.dataset.editId);
      });
    });
    ...
    page.querySelectorAll('[data-del-id]').forEach(function(btn){
      btn.addEventListener('click',function(e){
        ...
        var up=Preset.config.userPresets[id];if(!up)return;
        if(!confirm('删除「'+(up.name||'未命名')+'」？'))return;
        delete Preset.config.userPresets[id];
        delete Preset.config._savedPositions[id];
        var oi=Preset.config.order.indexOf(id);
        if(oi>=0)Preset.config.order.splice(oi,1);
        Preset.save();Preset.renderList(page);App.showToast('已删除');
      });
    });
    ...
    Preset._bindSwipeToEl(page,function(){Preset.closeDetail();});
  },
  ...
  bindDrag:function(page){
    ...
    list.addEventListener('touchstart',function(e){
      ...
      timer=setTimeout(function(){
        if(!dragEl||moved)return;longPressed=true;
        itemEls=[];itemRects=[];
        list.querySelectorAll('.ps-item:not(.is-inactive)').forEach(function(it){
          if(it===dragEl)return;
          itemEls.push(it);
          var rect=it.getBoundingClientRect();
          itemRects.push({id:it.dataset.id,mid:rect.top+rect.height/2});
        });
        dragEl.classList.add('dragging');
        if(navigator.vibrate)navigator.vibrate(15);
      },350);
    ...
    list.addEventListener('touchmove',function(e){
      if(!dragEl)return;var t=e.touches[0];
      if(!longPressed){
        if(Math.abs(t.clientX-startX)>8||Math.abs(t.clientY-startY)>8){clearTimeout(timer);timer=null;dragEl=null;}
        return;
      }
    ...
    list.addEventListener('touchend',function(e){
      ...
      var touchY=e.changedTouches[0].clientY;
      var newOrder=[];var insertDone=false;
      for(var i=0;i<itemRects.length;i++){
        if(!insertDone&&touchY<itemRects[i].mid){newOrder.push(dragId);insertDone=true;}
        newOrder.push(itemRects[i].id);
      }
      if(!insertDone)newOrder.push(dragId);
  ...
  _bindSwipeToEl:function(el,closeFn){
    var _s={active:false,sx:0,sy:0,locked:false,dir:'',edge:false};

    el.addEventListener('touchstart',function(e){
      var t=e.touches[0];
      _s={active:true,sx:t.clientX,sy:t.clientY,locked:false,dir:'',edge:t.clientX<=50};
    },{passive:true});

    el.addEventListener('touchmove',function(e){
      if(!_s.active||!_s.edge)return;
      var t=e.touches[0];
      var dx=t.clientX-_s.sx;
      var dy=t.clientY-_s.sy;
      if(!_s.locked){
        if(Math.abs(dx)<10&&Math.abs(dy)<10)return;
        _s.locked=true;
        _s.dir=Math.abs(dx)>Math.abs(dy)?'h':'v';
      }
      if(_s.dir==='h'&&dx>0){
        e.preventDefault();
    ...
    el.addEventListener('touchend',function(e){
      if(!_s.active){return;}
      _s.active=false;
      if(!_s.edge||_s.dir!=='h'){el.style.transform='';el.style.opacity='';return;}
      ...
      if(dx>w*0.3){
        ...
        el.style.opacity='0';
        setTimeout(function(){el.style.transition='';el.style.transform='';el.style.opacity='';closeFn();},260);
      }else{
  ...
  openEdit:function(editId){
    ...
    var ep=document.createElement('div');ep.id='presetEditPage';ep.className='ps-edit-page';
    ...
    ep.innerHTML=
      '<div class="ps-header">'+
        ...
        '<span class="ps-header-title">'+(isNew?'添加指令':'编辑指令')+'</span>'+
      ...
      '<div class="ps-edit-body">'+
        '<div class="ps-edit-card">'+
          '<div class="ps-edit-section">'+
            ...
            '<input type="text" class="ps-edit-input" id="psEditName" placeholder="给这条指令起个名字..." value="'+App.escAttr(up.name||'')+'">'+
          '</div>'+
          '<div class="ps-edit-sep"></div>'+
          '<div class="ps-edit-section">'+
            '<div class="ps-edit-label"><div class="dot"></div>指令内容</div>'+
            '<textarea class="ps-edit-textarea" id="psEditContent" placeholder="输入指令内容...">'+App.esc(up.content||'')+'</textarea>'+
          '</div>'+
          '<div class="ps-edit-sep"></div>'+
          '<div class="ps-edit-section">'+
            ...
            '<div id="psDepthRow" style="'+(currentMode==='depth'?'':'display:none;')+'">'+
              '<div class="ps-depth-row">'+
                '<input type="number" class="ps-depth-input" id="psEditDepth" min="0" max="50" value="'+(up.depth||0)+'">'+
                '<div class="ps-depth-hint">0 = 紧贴用户最新消息<br>数字越大，插入位置越靠前</div>'+
              '</div>'+
            '</div>'+
          '</div>'+
          '<div class="ps-edit-btns">'+
            '<button class="ps-save-btn" id="psEditSave" type="button">保 存</button>'+
            '<button class="ps-cancel-btn" id="psEditCancel" type="button">取 消</button>'+
          '</div>'+
        '</div>'+
      '</div>';
    ...
    function updateTip(){
      var tip=ep.querySelector('#psModeTip');
      tip.innerHTML=currentMode==='relative'
        ?'<div class="ps-edit-hint">穿插到列表中你滑动的位置，按排列顺序发送给模型。</div>'
        :'<div class="ps-edit-hint">注入到聊天历史中指定深度。深度越小越接近最新消息，AI越重视。</div>';
    }
    updateTip();

    ep.querySelectorAll('.ps-mode-btn').forEach(function(btn){
      btn.addEventListener('click',function(){
        ep.querySelectorAll('.ps-mode-btn').forEach(function(b){b.classList.remove('active');});
        btn.classList.add('active');
        currentMode=btn.dataset.mode;
        ep.querySelector('#psDepthRow').style.display=currentMode==='depth'?'':'none';
        updateTip();
      });
    });
    ...
    ep.querySelector('#psEditSave').addEventListener('click',function(){
      var name=ep.querySelector('#psEditName').value.trim();
      if(!name){App.showToast('请输入指令名称');return;}
      var content=ep.querySelector('#psEditContent').value.trim();
      if(!content){App.showToast('请输入指令内容');return;}
      var depth=parseInt(ep.querySelector('#psEditDepth').value)||0;
      ...
      if(isNew){
        ...
        if(oldMode==='depth'&&currentMode==='relative'){
          if(Preset.config.order.indexOf(editId)<0)Preset.config.order.unshift(editId);
        }
      }

      Preset.save();closeEdit();
      var lp=App.$('#presetPage');if(lp)Preset.renderList(lp);
      App.showToast(isNew?'已创建':'已保存');
    });

    Preset._bindSwipeToEl(ep,function(){closeEdit();});
