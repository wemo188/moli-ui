...
var MENU_SVG = '<div style="width:40px;height:40px;"><svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" width="40" height="40"><path d="M 24 22 L 24 58 M 6 40 L 42 40" stroke="#2a2a2a" stroke-width="5" stroke-linecap="round"/><circle cx="24" cy="40" r="4.5" fill="#2a2a2a"/><path d="M 50 10 L 50 30 M 40 20 L 60 20" stroke="#2a2a2a" stroke-width="4" stroke-linecap="round"/><circle cx="50" cy="20" r="3.5" fill="#2a2a2a"/><path d="M 14 8 L 14 20 M 8 14 L 20 14" stroke="#2a2a2a" stroke-width="3.5" stroke-linecap="round"/><circle cx="14" cy="14" r="3" fill="#2a2a2a"/></svg></div>';
...
h+='<div class="ol-inline-row"><span>颜色</span><div class="hp-color-dot" id="ol'+p+'Color" data-fk="'+p+'Color"></div></div><div class="hp-slider-row"><span class="hp-slider-label">字号</span><input type="range" data-fk="'+p+'Size" min="10" max="50" step="0.5" value="'+ap[p+'Size']+'"><span class="hp-slider-val" id="ol'+p+'SizeVal">'+ap[p+'Size']+'px</span></div><div class="hp-slider-row"><span class="hp-slider-label">字重</span><input type="range" data-fk="'+p+'Weight" min="100" max="900" step="100" value="'+ap[p+'Weight']+'"><span class="hp-slider-val" id="ol'+p+'WeightVal">'+ap[p+'Weight']+'</span></div><div class="ol-inline-tag-row"><span class="ol-sub-label">样式</span><div class="ol-tag-group">'+tg('ol-'+p+'-style','normal','正常',!ap[p+'Italic'])+tg('ol-'+p+'-style','italic','斜体',ap[p+'Italic'])+'</div></div>';return h;}
...
'<div class="hp-slider-row" style="margin-bottom:12px;"><span class="hp-slider-label" style="width:60px">期望字数</span><input type="number" id="olWordCount" placeholder="留空则由模型自己决定" value="'+(ap.wordCount||'')+'" style="flex:1;min-width:0;height:48px;padding:0 12px;box-sizing:border-box;border:1.5px solid #adcdea;border-radius:8px;font-size:14px;background:#fff;box-shadow:none;outline:none;"></div>' +
'<div class="ol-sw-row" style="margin-bottom:12px;">流式 '+sw('olStreamOn',ap.streamOn!==false)+'</div>' +
...
'<div class="hp-slider-row" style="margin-bottom:12px;"><span class="hp-slider-label" style="width:60px">圆角</span><input type="range" id="olBarRadius" min="0" max="50" value="'+ap.barRadius+'"><span class="hp-slider-val" id="olBarRadiusVal">'+ap.barRadius+'px</span></div>' +
...
var input=App.$('#olInput');if(input){input.addEventListener('input',function(){this.style.height='34px';this.style.height=Math.min(this.scrollHeight,140)+'px';});input.addEventListener('keydown',function(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();OL.sendUser();}});}
...
var swMap={'olPovOn':'povOn','olStreamOn':'streamOn','olcAvShow':'cAvShow','olcAvNameShow':'cAvNameShow','oluAvShow':'uAvShow','oluAvNameShow':'uAvNameShow','olcQuoteOn':'cQuoteOn','olcParenOn':'cParenOn','olcStarOn':'cStarOn','olcParenHide':'cParenHide','olcStarHide':'cStarHide','olquoteOn':'quoteOn','olparenOn':'parenOn','olstarOn':'starOn','olparenHide':'parenHide','olstarHide':'starHide'};
