...
var DEF_AP={
  ...
  cTextSize:15,cTextWeight:400,cTextLH:1.85,cTextColor:'#2e4258',
  ...
  textSize:15,textWeight:400,textLH:1.85,textColor:'#2e4258',
  quoteOn:false,quoteRec:['curly','straight'],quoteDis:'curly',quoteColor:'#2e4258',quoteSize:15,quoteWeight:400,quoteItalic:false,
  parenOn:false,parenRec:['full','half'],parenDis:'full',parenHide:false,parenColor:'#7a9ab8',parenSize:15,parenWeight:400,parenItalic:true,
  starOn:false,starHide:true,starColor:'#7a9ab8',starSize:15,starWeight:400,starItalic:true
};
...
var fontBody=
  ...
  '<div class="ol-fmt-section"><div class="ol-sub-title">双引号识别 '+swHtml('olquoteOn',ap.quoteOn)+'</div>'+quoteHint+
    ...
    '<div class="ol-tag-row"><span class="ol-sub-label" style="margin:0;">样式</span>'+tagHtml('ol-quote-style','normal','正常',!ap.quoteItalic)+tagHtml('ol-quote-style','italic','斜体',ap.quoteItalic)+'</div></div>'+
  '<div class="hp-divider"></div>'+

  '<div class="ol-fmt-section"><div class="ol-sub-title">括号识别 '+swHtml('olparenOn',ap.parenOn)+'</div>'+
    '<div class="ol-sub-label">识别符号（多选）</div><div class="ol-tag-row">'+
      tagHtml('ol-prec','full','（…）',ap.parenRec.indexOf('full')>=0)+tagHtml('ol-prec','half','(…)',ap.parenRec.indexOf('half')>=0)+
    '</div><div class="ol-sub-label">显示符号（单选）</div><div class="ol-tag-row">'+
      tagHtml('ol-pdis','full','（…）',ap.parenDis==='full')+tagHtml('ol-pdis','half','(…)',ap.parenDis==='half')+
    '</div>'+
    ...
    '<div class="ol-tag-row"><span class="ol-sub-label" style="margin:0;">样式</span>'+tagHtml('ol-paren-style','normal','正常',!ap.parenItalic)+tagHtml('ol-paren-style','italic','斜体',ap.parenItalic)+'</div></div>'+
  '<div class="hp-divider"></div>'+

  '<div class="ol-fmt-section"><div class="ol-sub-title">单星号识别 '+swHtml('olstarOn',ap.starOn)+'</div>'+
    ...
    '<div class="ol-tag-row"><span class="ol-sub-label" style="margin:0;">样式</span>'+tagHtml('ol-star-style','normal','正常',!ap.starItalic)+tagHtml('ol-star-style','italic','斜体',ap.starItalic)+'</div></div>'+
  '<div class="hp-divider"></div>'+
...
'<div id="olSettingsPanel" class="half-panel hidden">'+
  ...
  '<div class="hp-body">'+
    ...
    '<div id="olPovSub" style="'+(ap.povOn?'':'display:none;')+'">'+
      '<div class="ol-sub-label">称呼用户为</div>'+
      '<div class="hp-btn-row"><button class="hp-btn ol-povu-btn" data-pov="first">我</button><button class="hp-btn ol-povu-btn" data-pov="second">你</button><button class="hp-btn ol-povu-btn" data-pov="third">他/她</button></div>'+
      '<div class="ol-sub-label">称呼角色为</div>'+
      '<div class="hp-btn-row"><button class="hp-btn ol-povc-btn" data-pov="first">我</button><button class="hp-btn ol-povc-btn" data-pov="second">你</button><button class="hp-btn ol-povc-btn" data-pov="third">他/她</button></div>'+
    '</div>'+
    ...
    foldHtml('olFoldCfont','字体',charFontBody)+
    '<div class="hp-divider"></div>'+
    ...
    '<div style="font-size:13px;color:#1a1a1a;padding:10px 0;">敬请期待</div>'+
    '<div class="hp-divider"></div>'+
...
r.style.setProperty('--ol-c-text-size',ap.cTextSize+'px');
r.style.setProperty('--ol-c-text-weight',String(ap.cTextWeight));
r.style.setProperty('--ol-c-text-lh',String(ap.cTextLH));
r.style.setProperty('--ol-c-text-color',ap.cTextColor);
...
formatProse:function(rawText,cid){
  ...
  if(ap.starOn){
    text=text.replace(/\*([^*]+)\*/g,function(m,g){return protect('<span class="ol-fmt-star">'+(ap.starHide?'':'*')+App.esc(g)+(ap.starHide?'':'*')+'</span>');});
    text=text.replace(/＊([^＊]+)＊/g,function(m,g){return protect('<span class="ol-fmt-star">'+(ap.starHide?'':'＊')+App.esc(g)+(ap.starHide?'':'＊')+'</span>');});
  }
  if(ap.quoteOn){
    ...
    pairs.forEach(function(p){
      ...
      text=text.replace(re,function(m,g){return protect('<span class="ol-fmt-quote">'+App.esc(dq[0])+App.esc(g)+App.esc(dq[1])+'</span>');});
    });
  }
  if(ap.parenOn){
    var pMap={full:['（','）'],half:['(',')']};var dp=pMap[ap.parenDis]||pMap.full;
    var ppairs=[];
...
/* Long press: only on blank area, not on text */
var mc=App.$('#olMsgs');
