...
function hexToRgba(hex,alpha){if(!hex)return 'rgba(255,255,255,'+alpha+')';if(hex.indexOf('rgba')>=0){return hex.replace(/,\s*[\d.]+\s*\)/,','+alpha+')');}if(hex.indexOf('rgb')>=0){return hex.replace('rgb','rgba').replace(')',','+alpha+')');}hex=hex.replace('#','');if(hex.length===3)hex=hex[0]+hex[0]+hex[1]+hex[1]+hex[2]+hex[2];if(hex.length!==6)return 'rgba(255,255,255,'+alpha+')';var r=parseInt(hex.substr(0,2),16),g=parseInt(hex.substr(2,2),16),b=parseInt(hex.substr(4,2),16);return 'rgba('+r+','+g+','+b+','+alpha+')';}
...
var charAvBody='<div class="ol-sw-row">头像 '+swHtml('olcAvShow',ap.cAvShow)+'</div><div class="ol-sw-row">名称 '+swHtml('olcAvNameShow',ap.cAvNameShow)+'</div>'+
...
var charBubBody='<div class="ol-inline-row"><span>背景</span><div class="hp-color-dot" id="olcBubbleBg"></div></div>'+
...
var charFontBody='<div class="ol-sub-title">正文</div>'+
  ...
  '<div class="ol-inline-row"><span>颜色</span><div class="hp-color-dot" id="olcTextColor"></div></div><div class="hp-divider"></div>'+
...
var userAvBody='<div class="ol-sw-row">头像 '+swHtml('oluAvShow',ap.uAvShow)+'</div><div class="ol-sw-row">名称 '+swHtml('oluAvNameShow',ap.uAvNameShow)+'</div>'+
...
var userBubBody='<div class="ol-inline-row"><span>背景</span><div class="hp-color-dot" id="oluBubbleBg"></div></div>'+
...
var userFontBody='<div class="ol-sub-title">正文</div>'+
  ...
  '<div class="ol-inline-row"><span>颜色</span><div class="hp-color-dot" id="oltextColor"></div></div><div class="hp-divider"></div>'+
  ...
  '<div class="ol-fmt-section"><div class="ol-sub-title">星号 '+swHtml('olstarOn',ap.starOn)+'</div>'+buildFmtUI('star',ap)+'</div><div class="hp-divider"></div>'+
...
'<div id="olSettingsPanel" class="half-panel hidden"><div class="hp-handle"></div><div class="hp-header"><h2>设置</h2><button class="hp-close" id="olPanelClose" type="button"><svg viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12"/></svg></button></div><div class="hp-body">'+
  ...
  '<div class="hp-divider"></div><div class="ol-sub-title">角色板块</div>'+foldHtml('olFoldCav','头像',charAvBody)+foldHtml('olFoldCbub','气泡',charBubBody)+foldHtml('olFoldCfont','字体',charFontBody)+
  '<div class="hp-divider"></div><div class="ol-sub-title">用户板块</div>'+foldHtml('olFoldUav','头像',userAvBody)+foldHtml('olFoldUbub','气泡',userBubBody)+foldHtml('olFoldUfont','字体',userFontBody)+
...
r.style.setProperty('--ol-para-gap',ap.blockGap+'px');r.style.setProperty('--ol-letter-gap',ap.letterGap+'px');
...
if(wrap){var bv=ap.barBg;var alpha=(ap.barOpacity!=null?ap.barOpacity:100)/100;
  if(bv.indexOf('gradient')>=0){wrap.style.background=bv;wrap.style.opacity=alpha;}
...
formatProse:function(raw,cid,isUser){
  ...
  function fmtSpan(content,color,size,weight,italic){return '<span style="color:'+color+' !important;font-size:'+size+'px !important;font-weight:'+weight+' !important;font-style:'+(italic?'italic':'normal')+' !important;">'+content+'</span>';}

  function doStar(p){if(!ap[p+'On'])return;
    text=text.replace(/(?:\*|＊)([^*＊]+)(?:\*|＊)/g,function(_,g){var inner=ap[p+'Hide']?App.esc(g):('*'+App.esc(g)+'*');return protect(fmtSpan(inner,ap[p+'Color'],ap[p+'Size'],ap[p+'Weight'],ap[p+'Italic']));});}

  function doQuote(p){if(!ap[p+'On'])return;
    var qMap={curly:['\u201C','\u201D'],straight:['"','"'],corner:['「','」']};
    var dq=qMap[ap[p+'Dis']]||qMap.curly;var rec=ap[p+'Rec']||[];var pairs=[];
    if(rec.indexOf('curly')>=0)pairs.push(['\u201C','\u201D']);if(rec.indexOf('straight')>=0)pairs.push(['"','"']);if(rec.indexOf('corner')>=0)pairs.push(['「','」']);
    pairs.forEach(function(pr){var re=new RegExp(escRe(pr[0])+'([^'+escRe(pr[1])+']+)'+escRe(pr[1]),'g');
      text=text.replace(re,function(_,g){return protect(fmtSpan(App.esc(dq[0])+App.esc(g)+App.esc(dq[1]),ap[p+'Color'],ap[p+'Size'],ap[p+'Weight'],ap[p+'Italic']));});});}

  function doParen(p){if(!ap[p+'On'])return;
    var pMap={full:['（','）'],half:['(',')']};var dp=pMap[ap[p+'Dis']]||pMap.full;var rec=ap[p+'Rec']||[];var ppairs=[];
    if(rec.indexOf('full')>=0)ppairs.push(['（','）']);if(rec.indexOf('half')>=0)ppairs.push(['(',')']);
    ppairs.forEach(function(pr){var re=new RegExp(escRe(pr[0])+'([^'+escRe(pr[1])+']+)'+escRe(pr[1]),'g');
      text=text.replace(re,function(_,g){var inner=ap[p+'Hide']?App.esc(g):(App.esc(dp[0])+App.esc(g)+App.esc(dp[1]));return protect(fmtSpan(inner,ap[p+'Color'],ap[p+'Size'],ap[p+'Weight'],ap[p+'Italic']));});});}
...
OL.messages.forEach(function(msg,idx){
  ...
  /* ★ 气泡透明：只对背景层生效，文字不受影响 */
  var bubBg=isUser?ap.uBubbleBg:ap.cBubbleBg;
  var bubOpacity=(isUser?ap.uBubbleOpacity:ap.cBubbleOpacity);
  var bubBlur=isUser?ap.uBubbleBlur:ap.cBubbleBlur;
  var bgAlpha=(bubOpacity!=null?bubOpacity:100)/100;
  var blurStyle=(bubBlur>0)?'backdrop-filter:blur('+bubBlur+'px);-webkit-backdrop-filter:blur('+bubBlur+'px);':'';
  var bgLayerStyle='position:absolute;inset:0;z-index:0;pointer-events:none;border-radius:inherit;background:'+bubBg+';opacity:'+bgAlpha+';'+blurStyle;
  ...
  html+='<div class="ol-block'+(isUser?' is-user':' is-char')+' ol-av-'+avPos+'" data-msg-idx="'+idx+'"><div class="ol-avatar-area"><div class="ol-avatar-frame"><div class="ol-avatar">'+avHtml+'</div></div><div class="ol-avatar-name">'+avName+'</div></div><div class="ol-bubble-col"><div class="ol-frame-mid" style="background:transparent !important;"><div style="'+bgLayerStyle+'"></div><div class="ol-bubble-inner">'+thinkHtml+'<div class="ol-bubble-text">'+formatted+'</div></div></div>'+meta+'</div></div>';
});
...
var sliders=[
  ...
  {id:'olBlockGap',k:'blockGap',u:'px'},{id:'olLetterGap',k:'letterGap',u:'px'},
...
/* ★ fmt sliders + fmt colors: data-fmtkey */
App.$$('[data-fmtkey]').forEach(function(el){
  var k=el.dataset.fmtkey;
  if(el.tagName==='INPUT'&&el.type==='range'){
    var valEl=App.$('#ol'+k+'Val');
  ...
  if(el.classList.contains('hp-color-dot')){
    el.style.background=ap[k];
    el.addEventListener('click',function(e){e.stopPropagation();if(!App.openColorPicker)return;
      App.openColorPicker(ap[k],function(hex){el.style.background=hex;ap[k]=hex;sr();},function(hex){el.style.background=hex;ap[k]=hex;sAp(cid,ap);OfflineUI._noScroll=true;OfflineUI.renderMessages();},'ol_'+k);
    });
  }
});

/* ★ colors: onChange也保存到storage再apply */
...
/* ★ 重置：同时重置闭包里的 ap 变量 */
App.safeOn('#olStyleReset','click',function(){App.LS.remove('olAp_'+cid);ap=JSON.parse(JSON.stringify(DEF_AP));OfflineUI.applyAppearance(cid);OfflineUI._noScroll=true;OfflineUI.renderMessages();App.showToast('已重置');});

/* ★ 长按：整个 block 都触发菜单，不排除文字区 */
var mc=App.$('#olMsgs');
