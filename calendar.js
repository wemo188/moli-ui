...
renderDaysGrid:function(){var grid=App.$('#calDaysGrid');if(!grid)return;var year=Cal._viewYear,month=Cal._viewMonth,today=Cal.todayKey(),fd=new Date(year,month,1).getDay(),dim=new Date(year,month+1,0).getDate(),html='';for(var e=0;e<fd;e++)html+='<div class="cal-day-cell cal-day-empty"></div>';for(var d=1;d<=dim;d++){var dk=year+'-'+String(month+1).padStart(2,'0')+'-'+String(d).padStart(2,'0'),dow=new Date(year,month,d).getDay(),cls='cal-day-cell';if(dk===today)cls+=' cal-day-today';if(dk===Cal._selectedDate)cls+=' cal-day-selected';if(dow===0)cls+=' cal-day-sunday';if(dow===6)cls+=' cal-day-saturday';html+='<div class="'+cls+'" data-date="'+dk+'"><div class="cal-day-num">'+d+'</div>'+(Cal.hasMemosForDate(dk)?'<div class="cal-day-dot"></div>':'')+'</div>';}grid.innerHTML=html;var cells=grid.querySelectorAll('.cal-day-cell:not(.cal-day-empty)');for(var i=0;i<cells.length;i++){cells[i].addEventListener('click',(function(cell){return function(){document.querySelectorAll('.cal-day-cell').forEach(function(c){c.classList.remove('cal-day-selected');});cell.classList.add('cal-day-selected');Cal._selectedDate=cell.dataset.date;Cal._stickerPage=0;Cal.renderSelectedSection();};})(cells[i]));}},
