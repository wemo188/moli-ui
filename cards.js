  ...
  var Cards = {
      ...
      if (App.showImagePicker) {
        App.showImagePicker({
          ...
          callback: function(src) {
            if (src) {
              Cards._applyHlAvatar(src, bgId, storageKey);
            } else {
              Cards._clearHlAvatar(bgId, storageKey);
            }
            if (cb) cb(src);
          }
        });
    ...
    // 🌟 头像与气泡的悬浮拖拽
      ...
      el.addEventListener('touchend', function(e) {
        clearTimeout(timer); timer = null; el.classList.remove('hl-dragging');
    ...
    openHlEdit: function(side) {
      ...
      if (avatarEl) {
        var rect = avatarEl.getBoundingClientRect(); var left = rect.left - 60; var top = rect.bottom + 8;
      ...
      panel.querySelector('#hlSaveBtn').addEventListener('click', function(e) {
        ...
        var bubbleEl = document.getElementById(side === 'left' ? 'hlInputLeft' : 'hlInputRight');
        var chatEl = document.getElementById(side === 'left' ? 'hlChatLeft' : 'hlChatRight');
        if (bubbleEl) bubbleEl.textContent = bv;
        if (chatEl) chatEl.textContent = cv;
    ...
    openEdit: function(side, cardEl) {
      ...
      panel.querySelector('#pcUploadBtn').addEventListener('click', function(e) {
        ...
        if (App.showImagePicker) {
          App.showImagePicker({
            title: '设置卡片头像',
            deleteText: '删除头像',
            callback: function(src) { tempAvatar = src; previewAvatar(); }
          });
        } else {
          var input = document.createElement('input'); input.type = 'file'; input.accept = 'image/*';
      ...
      overlay.addEventListener('click', function(e) { if (e.target === overlay && !document.querySelector('.crop-overlay') && !document.querySelector('.gip-overlay')) closeAndRevert(); });
    },
