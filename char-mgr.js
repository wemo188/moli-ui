...
(function() {
  ...
  var CharMgr = {
    ...
    getEffective: function(charId) {
      if (!charId) return CharMgr.globalConfig;
    ...
    render: function(page) {
      ...
      var cfg = CharMgr.currentTab === 'global' ? CharMgr.globalConfig : CharMgr.getCharConfig(CharMgr.selectedCharId);
      if (!cfg) cfg = CharMgr.globalConfig;
      ...
      var biChecked = cfg.bilingual ? ' checked' : '';
      var mmChecked = cfg.minimax ? ' checked' : '';
      var fallChecked = cfg.fallbackTTS ? ' checked' : '';
      var proChecked = cfg.proactive ? ' checked' : '';
      var twChecked = cfg.timeWeather ? ' checked' : '';
      var stkChecked = cfg.stickerGen ? ' checked' : '';
      var momChecked = cfg.moments ? ' checked' : '';
      ...
      // API list
      var apiList = App.LS.get('apiList') || [];
      ...
      apiOptionsHtml += '<option value="__new__">+ 新建 API 配置...</option>';

      var proLevelNames = ['佛系','偶尔','适中','频繁','粘人'];
      var stkFreqNames = ['极少','偶尔','适中','经常','频繁'];
      ...
      page.innerHTML =
        '<div style="display:flex;align-items:center;justify-content:space-between;padding:56px 16px 12px;flex-shrink:0;background:#fff;border-bottom:1px solid #eee;">' +
          '<button class="cm-back-btn" id="cmBackBtn" type="button" style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;background:none;border:none;cursor:pointer;-webkit-tap-highlight-color:transparent;"><svg viewBox="0 0 24 24" style="width:20px;height:20px;fill:none;stroke:#7a9ab8;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;"><path d="M19 12H5M12 5l-7 7 7 7"/></svg></button>' +
        ...
        '<div style="flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;">' +
          ...
          '<div class="cm-char-row" id="cmCharRow">' + charRowHtml + '</div>' +
          ...
          '<div class="cm-comic"><div class="cm-comic-bar"></div><div class="cm-section"><div class="cm-section-head"><div class="cm-section-title cm-green">语言与语音</div></div><div class="cm-section-body">' +
            '<div class="cm-field" style="margin-bottom:12px"><div class="cm-field-label">主要语言</div><select class="cm-select" id="cmMainLang"><option' + (cfg.mainLang==='简体中文'?' selected':'') + '>简体中文</option><option' + (cfg.mainLang==='繁體中文'?' selected':'') + '>繁體中文</option><option' + (cfg.mainLang==='粤语'?' selected':'') + '>粤语</option><option' + (cfg.mainLang==='English'?' selected':'') + '>English</option><option' + (cfg.mainLang==='日本語'?' selected':'') + '>日本語</option><option' + (cfg.mainLang==='한국어'?' selected':'') + '>한국어</option></select></div>' +
            '<div class="cm-sep"></div>' +
            '<div class="cm-sw-row"><div class="cm-sw-left"><span class="cm-sw-name">双语模式</span><span class="cm-sw-desc">每条消息附带翻译</span></div><label class="cm-sw"><input type="checkbox" id="cmBiToggle"' + biChecked + '><div class="cm-sw-track"></div></label></div>' +
            '<div class="cm-sub' + (cfg.bilingual ? ' cm-open' : '') + '" id="cmBiSub">' +
              '<div class="cm-sub-row"><div class="cm-field"><div class="cm-field-label">翻译为</div><select class="cm-select" id="cmBiLang"><option' + (cfg.biLang==='English'?' selected':'') + '>English</option><option' + (cfg.biLang==='日本語'?' selected':'') + '>日本語</option><option' + (cfg.biLang==='한국어'?' selected':'') + '>한국어</option><option' + (cfg.biLang==='繁體中文'?' selected':'') + '>繁體中文</option><option' + (cfg.biLang==='粤语'?' selected':'') + '>粤语</option></select></div></div>' +
            ...
            '</div>' +
            '<div class="cm-sep"></div>' +
            '<div class="cm-sw-row"><div class="cm-sw-left"><span class="cm-sw-name">MiniMax 语音</span><span class="cm-sw-desc">使用 MiniMax TTS 生成角色语音</span></div><label class="cm-sw"><input type="checkbox" id="cmMmToggle"' + mmChecked + '><div class="cm-sw-track"></div></label></div>' +
            ...
            '</div>' +
            '<div class="cm-sep"></div>' +
            '<div class="cm-sw-row" style="border-bottom:none"><div class="cm-sw-left"><span class="cm-sw-name">备用 TTS 引擎</span><span class="cm-sw-desc">MiniMax 不可用时的替代方案</span></div><label class="cm-sw"><input type="checkbox" id="cmFallToggle"' + fallChecked + '><div class="cm-sw-track"></div></label></div>' +
            '<div class="cm-sub' + (cfg.fallbackTTS ? ' cm-open' : '') + '" id="cmFallSub">' +
              '<div class="cm-sub-row"><div class="cm-field"><div class="cm-field-label">TTS 引擎</div><select class="cm-select" id="cmFallEngine"><option' + (cfg.fallbackEngine==='Edge TTS'?' selected':'') + '>Edge TTS</option><option' + (cfg.fallbackEngine==='系统 Web Speech'?' selected':'') + '>系统 Web Speech</option><option' + (cfg.fallbackEngine==='OpenAI TTS'?' selected':'') + '>OpenAI TTS</option></select></div></div>' +
          ...
          '<div class="cm-comic"><div class="cm-comic-bar"></div><div class="cm-section"><div class="cm-section-head"><div class="cm-section-title cm-blue">消息行为</div></div><div class="cm-section-body">' +
            '<div class="cm-sw-row"><div class="cm-sw-left"><span class="cm-sw-name">主动发消息</span><span class="cm-sw-desc">角色会不定时主动联系你</span></div><label class="cm-sw"><input type="checkbox" id="cmProToggle"' + proChecked + '><div class="cm-sw-track"></div></label></div>' +
            '<div class="cm-sub' + (cfg.proactive ? ' cm-open' : '') + '" id="cmProSub">' +
              '<div class="cm-sub-label">消息频率</div>' +
              '<div style="display:flex;gap:14px"><div class="cm-field" style="flex:1"><div class="cm-field-label">最短间隔</div><div style="display:flex;align-items:center;gap:6px;margin-top:4px"><div class="cm-stepper"><button class="cm-stepper-btn" data-dir="-1" data-target="cmProMin" type="button">−</button><div class="cm-stepper-val" id="cmProMin">' + cfg.proMinInterval + '</div><button class="cm-stepper-btn" data-dir="1" data-target="cmProMin" type="button">+</button></div><span style="font-size:10px;color:#888">分钟</span></div></div>' +
              '<div class="cm-field" style="flex:1"><div class="cm-field-label">最长间隔</div><div style="display:flex;align-items:center;gap:6px;margin-top:4px"><div class="cm-stepper"><button class="cm-stepper-btn" data-dir="-1" data-target="cmProMax" type="button">−</button><div class="cm-stepper-val" id="cmProMax">' + cfg.proMaxInterval + '</div><button class="cm-stepper-btn" data-dir="1" data-target="cmProMax" type="button">+</button></div><span style="font-size:10px;color:#888">分钟</span></div></div></div>' +
              '<div class="cm-sub-label" style="margin-top:10px">活跃时段</div>' +
              '<div class="cm-time-group" style="margin-top:2px"><input type="text" class="cm-time-input" id="cmProStart" value="' + App.esc(cfg.proActiveStart) + '"><span class="cm-time-sep">至</span><input type="text" class="cm-time-input" id="cmProEnd" value="' + App.esc(cfg.proActiveEnd) + '"></div>' +
            ...
            '</div>' +
            '<div class="cm-sep"></div>' +
            '<div class="cm-field" style="margin-bottom:6px"><div class="cm-field-label">回复速度</div><select class="cm-select" id="cmReplySpeed"><option' + (cfg.replySpeed==='即时回复'?' selected':'') + '>即时回复</option><option' + (cfg.replySpeed==='快速（1-3秒）'?' selected':'') + '>快速（1-3秒）</option><option' + (cfg.replySpeed==='正常（3-8秒）'?' selected':'') + '>正常（3-8秒）</option><option' + (cfg.replySpeed==='慢速（5-15秒）'?' selected':'') + '>慢速（5-15秒）</option><option' + (cfg.replySpeed==='真实模拟（按字数）'?' selected':'') + '>真实模拟（按字数）</option></select></div>' +
          ...
          '<div class="cm-comic"><div class="cm-comic-bar"></div><div class="cm-section"><div class="cm-section-head"><div class="cm-section-title cm-purple">社交功能</div></div><div class="cm-section-body">' +
            ...
            '<div class="cm-sep"></div>' +
            '<div class="cm-sw-row"><div class="cm-sw-left"><span class="cm-sw-name">表情包生成</span><span class="cm-sw-desc">AI 生成自定义表情包图片</span></div><label class="cm-sw"><input type="checkbox" id="cmStkToggle"' + stkChecked + '><div class="cm-sw-track"></div></label></div>' +
            '<div class="cm-sub' + (cfg.stickerGen ? ' cm-open' : '') + '" id="cmStkSub">' +
              '<div class="cm-sub-row"><div class="cm-field"><div class="cm-field-label">生成引擎</div><select class="cm-select" id="cmStkEngine"><option>DALL·E 3</option><option>Stable Diffusion</option><option>MidJourney API</option></select></div></div>' +
              ...
              '<div class="cm-sub-row" style="margin-top:8px"><div class="cm-sub-label">发送频率</div><div class="cm-range-wrap"><span class="cm-range-hint">少</span><input type="range" class="cm-range" id="cmStkFreq" min="1" max="5" step="1" value="' + cfg.stickerFreq + '"><span class="cm-range-val" id="cmStkFreqVal">' + stkFreqNames[cfg.stickerFreq - 1] + '</span><span class="cm-range-hint">多</span></div></div>' +
            '</div>' +
            '<div class="cm-sep"></div>' +
            '<div class="cm-sw-row" style="border-bottom:none"><div class="cm-sw-left"><span class="cm-sw-name">朋友圈生成</span><span class="cm-sw-desc">角色自动发朋友圈动态</span></div><label class="cm-sw"><input type="checkbox" id="cmMomToggle"' + momChecked + '><div class="cm-sw-track"></div></label></div>' +
            '<div class="cm-sub' + (cfg.moments ? ' cm-open' : '') + '" id="cmMomSub">' +
              '<div class="cm-sub-row"><div class="cm-field"><div class="cm-field-label">每日最多发布</div><div style="display:flex;align-items:center;gap:6px;margin-top:4px"><div class="cm-stepper"><button class="cm-stepper-btn" data-dir="-1" data-target="cmMomMax" type="button">−</button><div class="cm-stepper-val" id="cmMomMax">' + cfg.momentsMax + '</div><button class="cm-stepper-btn" data-dir="1" data-target="cmMomMax" type="button">+</button></div><span style="font-size:10px;color:#888">条/天</span></div></div></div>' +
              ...
              '<div class="cm-sub-row" style="margin-top:8px"><div class="cm-field"><div class="cm-field-label">配图生成</div><select class="cm-select" id="cmMomImg"><option' + (cfg.momentsImg==='不生成配图'?' selected':'') + '>不生成配图</option><option' + (cfg.momentsImg==='AI 生成'?' selected':'') + '>AI 生成</option><option' + (cfg.momentsImg==='使用预设图库'?' selected':'') + '>使用预设图库</option></select></div></div>' +
            '</div>' +
          '</div></div><div class="cm-comic-bar-bot"></div></div>' +
          ...
          '<div class="cm-comic"><div class="cm-comic-bar"></div><div class="cm-section"><div class="cm-section-head"><div class="cm-section-title cm-red">情境感知</div></div><div class="cm-section-body">' +
            '<div class="cm-sw-row" style="border-bottom:none"><div class="cm-sw-left"><span class="cm-sw-name">时间 & 天气感知</span><span class="cm-sw-desc">角色知道当前时间和天气并自然融入对话</span></div><label class="cm-sw"><input type="checkbox" id="cmTwToggle"' + twChecked + '><div class="cm-sw-track"></div></label></div>' +
          ...
          '<div class="cm-comic"><div class="cm-comic-bar"></div><div class="cm-section"><div class="cm-section-head"><div class="cm-section-title">高级设定</div><div class="cm-section-badge">ADVANCED</div></div><div class="cm-section-body">' +
            ...
            '<div class="cm-field" style="margin-bottom:4px"><div class="cm-field-label">选择已保存的 API</div><select class="cm-select" id="cmApiSelect">' + apiOptionsHtml + '</select></div>' +
            '<div class="cm-sep"></div>' +
    ...
    collectConfig: function(page) {
      ...
      return {
        mainLang: (page.querySelector('#cmMainLang') || {}).value || '简体中文',
        bilingual: (page.querySelector('#cmBiToggle') || {}).checked || false,
        biLang: (page.querySelector('#cmBiLang') || {}).value || 'English',
        biStyle: biStyleEl ? biStyleEl.value : 'bracket',
        minimax: (page.querySelector('#cmMmToggle') || {}).checked || false,
        mmVoiceId: (page.querySelector('#cmMmVoice') || {}).value || '',
        mmApiKey: (page.querySelector('#cmMmKey') || {}).value || '',
        mmSpeed: parseFloat((page.querySelector('#cmMmSpeed') || {}).value || 1),
        mmPitch: parseInt((page.querySelector('#cmMmPitch') || {}).value || 0),
        fallbackTTS: (page.querySelector('#cmFallToggle') || {}).checked || false,
        fallbackEngine: (page.querySelector('#cmFallEngine') || {}).value || 'Edge TTS',
        fallbackVoice: (page.querySelector('#cmFallVoice') || {}).value || '',
        proactive: (page.querySelector('#cmProToggle') || {}).checked || false,
        proMinInterval: parseInt((page.querySelector('#cmProMin') || {}).textContent || 15),
        proMaxInterval: parseInt((page.querySelector('#cmProMax') || {}).textContent || 120),
        proActiveStart: (page.querySelector('#cmProStart') || {}).value || '08:00',
        proActiveEnd: (page.querySelector('#cmProEnd') || {}).value || '23:30',
        proMode: proModeEl ? proModeEl.value : 'manual',
        proLevel: parseInt((page.querySelector('#cmProLevel') || {}).value || 3),
        replySpeed: (page.querySelector('#cmReplySpeed') || {}).value || '正常（3-8秒）',
        showTyping: (page.querySelector('#cmShowTyping') || {}).value === '显示',
        msgTypes: msgTypes,
        stickerGen: (page.querySelector('#cmStkToggle') || {}).checked || false,
        stickerEngine: (page.querySelector('#cmStkEngine') || {}).value || 'DALL·E 3',
        stickerStyle: (page.querySelector('#cmStkStyle') || {}).value || '可爱卡通',
        stickerFreq: parseInt((page.querySelector('#cmStkFreq') || {}).value || 2),
        moments: (page.querySelector('#cmMomToggle') || {}).checked || false,
        momentsMax: parseInt((page.querySelector('#cmMomMax') || {}).textContent || 2),
        momentsTypes: momTypes,
        momentsImg: (page.querySelector('#cmMomImg') || {}).value || '不生成配图',
        timeWeather: (page.querySelector('#cmTwToggle') || {}).checked || false,
        charCity: (page.querySelector('#cmCharCity') || {}).value || '',
        userCity: (page.querySelector('#cmUserCity') || {}).value || '',
        apiMode: (page.querySelector('#cmApiMode') || {}).value || 'global',
        apiSelect: (page.querySelector('#cmApiSelect') || {}).value || '',
        temperature: parseFloat((page.querySelector('#cmTemp') || {}).value || 0.8),
        freqPenalty: parseFloat((page.querySelector('#cmFreq') || {}).value || 0.3),
        presPenalty: parseFloat((page.querySelector('#cmPres') || {}).value || 0.3)
      };
    },
    ...
    bindEvents: function(page) {
      ...
      function bindToggle(toggleId, subId) {
        var toggle = page.querySelector('#' + toggleId);
        var sub = page.querySelector('#' + subId);
        if (toggle && sub) {
          toggle.addEventListener('change', function() {
            if (this.checked) sub.classList.add('cm-open');
            else sub.classList.remove('cm-open');
          });
        }
      }
      ...
      page.querySelectorAll('input[name="cmProMode"]').forEach(function(radio) {
        radio.addEventListener('change', function() {
          var manual = page.querySelector('#cmProManual');
          var auto = page.querySelector('#cmProAuto');
      ...
      function bindRange(inputId, valId, formatter) {
        ...
        if (input && val) {
          input.addEventListener('input', function() { val.textContent = formatter(this.value); });
        }
      }
      bindRange('cmMmSpeed', 'cmMmSpeedVal', function(v) { return v + 'x'; });
      bindRange('cmMmPitch', 'cmMmPitchVal', function(v) { return (v > 0 ? '+' : '') + v; });
      bindRange('cmProLevel', 'cmProLevelVal', function(v) { return proLevelNames[v - 1]; });
      bindRange('cmStkFreq', 'cmStkFreqVal', function(v) { return stkFreqNames[v - 1]; });
      ...
      // Stepper
      page.querySelectorAll('.cm-stepper-btn').forEach(function(btn) {
        btn.addEventListener('click', function(e) {
          e.stopPropagation();
          var dir = parseInt(btn.dataset.dir);
          var target = btn.dataset.target;
          var valEl = page.querySelector('#' + target);
          if (!valEl) return;
          var v = parseInt(valEl.textContent) + dir;
          if (target === 'cmProMin') v = Math.max(1, Math.min(120, v));
          else if (target === 'cmProMax') v = Math.max(1, Math.min(480, v));
          else if (target === 'cmMomMax') v = Math.max(0, Math.min(10, v));
          valEl.textContent = v;
        });
      });

      // API 选择
      var apiSelect = page.querySelector('#cmApiSelect');
      if (apiSelect) {
        apiSelect.addEventListener('change', function() {
          if (this.value === '__new__') {
            CharMgr.close();
            setTimeout(function() { App.openPanel('apiPanel'); }, 380);
          }
        });
      }
